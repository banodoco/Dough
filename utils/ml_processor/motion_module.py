...
class AnimateDiffCheckpoint:
    ...
    @staticmethod
    def get_name_list():
        checkpoint_names = [getattr(AnimateDiffCheckpoint, attr).name for attr in dir(AnimateDiffCheckpoint) if not \
                            callable(getattr(AnimateDiffCheckpoint, attr)) and not attr.startswith("__") \
                                and isinstance(getattr(AnimateDiffCheckpoint, attr), MotionModuleCheckpoint)]
        return checkpoint_names
    
    @staticmethod
    def get_model_from_name(name):
        checkpoint_list = [getattr(AnimateDiffCheckpoint, attr) for attr in dir(AnimateDiffCheckpoint) if not \
                            callable(getattr(AnimateDiffCheckpoint, attr)) and not attr.startswith("__") \
                                and isinstance(getattr(AnimateDiffCheckpoint, attr), MotionModuleCheckpoint)]
