...
class DataRepo:
    ...
    def get_file_from_name(self, name):
        file = self.db_repo.get_file_from_name(name).payload
    ...
    def create_or_update_file(self, filename, type=InternalFileType.IMAGE.value, **kwargs):
        file = self.db_repo.create_or_update_file(filename, type, **kwargs).payload
        return InternalFileObject(file) if file else None
    
    def create_file(self, **kwargs):
        file = self.db_repo.create_file(**kwargs).payload
        return InternalFileObject(file) if file else None
    ...
    def get_timing_from_uuid(self, uuid):
        timing = self.db_repo.get_timing_from_uuid(uuid).payload
    ...
    def get_timing_from_frame_number(self, project_uuid, frame_number):
        timing = self.db_repo.get_timing_from_frame_number(project_uuid, frame_number).payload
        return InternalFrameTimingObject(timing) if timing else None
    
    def get_primary_variant_location(self, uuid):
        location = self.db_repo.get_primary_variant_location(uuid).payload
        return location
    
    # this is based on the aux_frame_index and not the order in the db
    def get_next_timing(self, uuid):
        next_timing = self.db_repo.get_next_timing(uuid).payload
    ...
    def create_timing(self, **kwargs):
        timing = self.db_repo.create_timing(**kwargs).payload
        return InternalFrameTimingObject(timing) if timing else None
    ...
    def get_app_setting_from_uuid(self, uuid):
        app_setting = self.db_repo.get_app_setting_from_uuid(uuid).payload
    ...
    def get_project_setting(self, project_id):
        project_setting = self.db_repo.get_project_from_uuid(project_id).payload
