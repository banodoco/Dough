...
def move_frame(project_name, index_of_current_item, distance_to_move, timing_details, input_video):
    current_frame_number = int(calculate_frame_number_at_time(
    ...
    if distance_to_move == 0:
        extract_frame(index_of_current_item, project_name,
                      input_video, current_frame_number, timing_details)
        new_frame_number = current_frame_number
    elif distance_to_move > 0:
        next_frame_number = int(calculate_frame_number_at_time(
            input_video, timing_details[index_of_current_item + 1]["frame_time"], project_name))
        abs_distance_to_move = abs(distance_to_move) / 100
        difference_between_frames = abs(
            next_frame_number - current_frame_number)
        new_frame_number = current_frame_number + \
            (difference_between_frames * abs_distance_to_move)
        extract_frame(index_of_current_item, project_name,
                      input_video, new_frame_number, timing_details)
    elif distance_to_move < 0:
        last_frame_number = int(calculate_frame_number_at_time(
            input_video, timing_details[index_of_current_item - 1]["frame_time"], project_name))
        abs_distance_to_move = abs(distance_to_move) / 100
        difference_between_frames = abs(
    ...
    new_time = calculate_time_at_frame_number(
        input_video, new_frame_number, project_name)
...
def trigger_restyling_process(timing_details, project_name, index_of_current_item, model, prompt, strength, custom_pipeline, negative_prompt, guidance_scale, seed, num_inference_steps, which_stage_to_run_on, promote_new_generation, project_settings, custom_models, adapter_type):
    ...
    if timing_details[index_of_current_item]["source_image"] == "":
        source_image = upload_image("videos/" + str(project_name) +
                                    "/assets/frames/1_selected/" + str(index_of_current_item) + ".png")
    else:
...
def prompt_interpolation_model(img1, img2, project_name, video_number, interpolation_steps, replicate_api_key):
    ...
    output = model.predict(frame1=img1, frame2=img2,
                           times_to_interpolate=interpolation_steps)
    file_name = ''.join(random.choices(
    ...
    update_specific_timing_value(
        project_name, video_number, "interpolated_video", video_location)
    update_specific_timing_value(
        project_name, video_number, "timing_video", "")
...
def prompt_model_stability(project_name, index_of_current_item, timing_details, input_image):
    ...
    if not input_image.startswith("http"):
        input_image = open(input_image, "rb")
    output = version.predict(image=input_image, prompt_strength=float(strength), prompt=prompt, negative_prompt=timing_details[index_of_current_item]["negative_prompt"], width=int(project_settings["width"]), height=int(
        project_settings["height"]), guidance_scale=float(timing_details[index_of_current_item]["guidance_scale"]), seed=int(timing_details[index_of_current_item]["seed"]), num_inference_steps=int(timing_details[index_of_current_item]["num_inference_steps"]))
    new_image = "videos/" + \
        str(project_name) + "/assets/frames/2_character_pipeline_completed/" + \
        str(index_of_current_item) + ".png"
...
def prompt_model_dreambooth(project_name, image_number, model_name, app_settings, timing_details, project_settings, image_url):
    ...
    if not input_image.startswith("http"):
        input_image = open(input_image, "rb")
    output = version.predict(image=input_image, prompt=prompt, prompt_strength=float(strength), height=int(project_settings["height"]), width=int(
        project_settings["width"]), disable_safety_check=True, negative_prompt=negative_prompt, guidance_scale=float(guidance_scale), seed=int(seed), num_inference_steps=int(num_inference_steps))
...
def restyle_images(index_of_current_item, project_name, project_settings, timing_details, source_image):
    ...
    if model_name == "stable-diffusion-img2img-v2.1":
        ...
        output_url = prompt_model_dreambooth(
            project_name, index_of_current_item, timing_details[index_of_current_item]["custom_models"], app_settings, timing_details, project_settings, source_image)
    elif model_name == 'StyleGAN-NADA':
        output_url = prompt_model_stylegan_nada(
            index_of_current_item, timing_details, source_image, project_name)
    elif model_name == "dreambooth_controlnet":
        output_url = prompt_model_dreambooth_controlnet(
            source_image, timing_details, project_name, index_of_current_item)
...
def create_timings_row_at_frame_number(project_name, input_video, extract_frame_number, timing_details, index_of_new_item):
    ...
    frame_time = calculate_time_at_frame_number(
        input_video, float(extract_frame_number), project_name)
...
def render_video(project_name, final_video_name):
    ...
    for i in timing_details:
        ...
        if index_of_current_item <= total_number_of_videos:
            if timing_details[index_of_current_item]["timing_video"] == "":
                total_duration_of_clip, duration_of_static_time = find_duration_of_clip(
                    index_of_current_item, timing_details, total_number_of_videos)
                update_video_speed(project_name, index_of_current_item,
                                   duration_of_static_time, total_duration_of_clip, timing_details)
    ...
    finalclip.write_videofile(
        output_video_file, fps=60, audio_bitrate="1000k", bitrate="4000k", codec="libx264")
    ...
    for i in range(depth_map.size[0]):
        for j in range(depth_map.size[1]):
            ...
            if layer == "Foreground":
                ...
                mask_pixels[i, j] = 0 if depth_value > 200 else 255
            elif layer == "Middleground":
                # Set middleground pixels to black
                mask_pixels[i, j] = 0 if depth_value <= 200 and depth_value > 50 else 255
            elif layer == "Background":
                # Set background pixels to black
                mask_pixels[i, j] = 0 if depth_value <= 50 else 255
...
def prompt_model_dreambooth_controlnet(input_image, timing_details, project_name, index_of_current_item):
    ...
    while completed == "false":
        ...
        if response.json()["status"] == "processing":
            ...
            time.sleep(wait)
            response = "https://stablediffusionapi.com/api/v3/dreambooth/fetch/" + \
...
def prompt_model_lora(project_name, index_of_current_item, timing_details, source_image):
    ...
    lora_models = ast.literal_eval(
        timing_details[index_of_current_item]["custom_models"][1:-1])
    project_settings = get_project_settings(project_name)
