...
def timeline_view(project_uuid, stage):
    ...
    with header_col_2:
        ...
        if expand_all:
            time_setter_toggle = replace_image_widget_toggle = duration_setter_toggle = move_frames_toggle = delete_frames_toggle = change_position_toggle = True
            ...
            if i + j < total_count:  # Check if index is within range
                with grid[j]:
                    ...
                    with st.expander(f'Frame #{display_number}', True):    
                        timeline_view_buttons(i, j, timing_details, shift_frames_setting, time_setter_toggle, replace_image_widget_toggle, duration_setter_toggle, move_frames_toggle, delete_frames_toggle, change_position_toggle)
