...
def shot_keyframe_element(shot_uuid, items_per_row, column=None, position="Timeline", **kwargs):
    ...
    with column:
        ...
        with col1:
            open_frame_changer = st_memory.toggle("Open Frame Changer™", value=False, key=f"open_frame_changer_{shot.uuid}", help="Enable to move frames around")
            if st.session_state[f"open_frame_changer_{shot.uuid}"]:
                ...
                if st.session_state[f"shot_data_{shot_uuid}"] is None:
                    shot_data = [{
                        "uuid": timing.uuid,
                        "image_uuid": timing.primary_image.uuid if timing.primary_image and timing.primary_image.uuid else None,
                    ...
                    with grid[j % items_per_row]:
                        ...
                        with btn2:
                            if st.button("➡️", key=f"move_frame_forward_{idx}", help="Move frame forward", use_container_width=True):
                                ...
                                st.rerun()
                        with btn3:
                            if st.button("🔁", key=f"copy_frame_{idx}", help="Duplicate frame", use_container_width=True):
                                st.session_state[f"shot_data_{shot_uuid}"] = copy_temp_frame(st.session_state[f"shot_data_{shot_uuid}"], idx)
                                st.rerun()
                        with btn4:
                            if st.button("❌", key=f"delete_frame_{idx}", help="Delete frame", use_container_width=True):
                    ...
                    with grid[j]:
                        if idx == len(timing_list):
                            ...
                            if timing.primary_image and timing.primary_image.location:
                                ...
                                jump_to_single_frame_view_button(idx + 1, timing_list, f"jump_to_{idx + 1}",uuid=shot_uuid)
                            else:                        
                                st.warning("No primary image present.")
...
def delete_shot_button(shot_uuid):
    ...
    help_text = "Check the box above to enable the delete button." if not confirm_delete else "This will delete this shot and all the frames and videos within."
...
def update_shot_frames(shot_uuid):
    ...
    timing_list = data_repo.get_timing_list_from_shot(shot_uuid)
    data_repo.update_bulk_timing([timing.uuid for timing in timing_list], [{'is_disabled': True}] * len(timing_list))
