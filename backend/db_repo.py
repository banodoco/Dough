...
from backend.serializers.dto import (
    AIModelDto,
    AppSettingDto,
    BackupDto,
    BackupListDto,
...
from backend.serializers.dao import (
    CreateAIModelDao,
    CreateAIModelParamMapDao,
    CreateAppSettingDao,
    CreateFileDao,
...
class DBRepo:
    ...
    def create_user(self, **kwargs):
        ...
        payload = {"data": UserDto(user).data}
    ...
    def get_all_user_list(self):
        ...
        payload = {"data": UserDto(user_list, many=True).data}
    ...
    def get_all_file_list(self, **kwargs):
        ...
        if "page" in kwargs and kwargs["page"]:
            ...
            payload = {"data": InternalFileDto(file_list, many=True).data}
    ...
    def get_file_list_from_log_uuid_list(self, log_uuid_list):
        ...
        file_list = InternalFileObject.objects.filter(
            inference_log__uuid__in=[str(log.uuid) for log in inference_log_list], is_disabled=False
        ).all()
        payload = {"data": InternalFileDto(file_list, many=True).data}
    ...
    def create_or_update_file(self, file_uuid, type=InternalFileType.IMAGE.value, **kwargs):
        ...
        if not file:
            file = InternalFileObject.objects.create(
                uuid=file_uuid, name=str(uuid.uuid4()), file_type=type, **kwargs
            )
        else:
    ...
    def create_file(self, **kwargs):
        ...
        if "hosted_url" not in kwargs and AUTOMATIC_FILE_HOSTING:
            ...
            hosted_url = upload_file(
                filename, app_setting.aws_access_key_decrypted, app_setting.aws_secret_access_key_decrypted
            )
        ...
        if "inference_log_id" in kwargs and kwargs["inference_log_id"]:
            inference_log = InferenceLog.objects.filter(
                uuid=kwargs["inference_log_id"], is_disabled=False
            ).first()
    ...
    def get_image_list_from_uuid_list(self, uuid_list, file_type=InternalFileType.IMAGE.value):
        file_list = InternalFileObject.objects.filter(
            uuid__in=uuid_list, is_disabled=False, type=file_type
        ).all()
    ...
    def get_file_count_from_type(self, file_tag, project_uuid):
        ...
        file_count = InternalFileObject.objects.filter(
            tag=file_tag, project_id=project.id, is_disabled=False
        ).count()
        payload = {"data": file_count}
    ...
    def update_temp_gallery_images(self, project_uuid):
        ...
        InternalFileObject.objects.filter(
            tag=InternalFileTag.TEMP_GALLERY_IMAGE.value, project_id=project.id, is_disabled=False
        ).update(tag=InternalFileTag.GALLERY_IMAGE.value)
    ...
    def get_project_from_uuid(self, uuid):
        ...
        payload = {"data": ProjectDto(project).data}
    ...
    def get_all_project_list(self, user_uuid):
        ...
        payload = {"data": ProjectDto(project_list, many=True).data}
    ...
    def get_all_ai_model_list(
        self, model_category_list=None, user_id=None, custom_trained=False, model_type_list=None
    ):
        query = {
            "custom_trained": (
        ...
        for model in ai_model_list:
            category_check = (
                True
                if (
                    not model_category_list or (model_category_list and model.category in model_category_list)
                )
                else False
            )
            type_check = (
                ...
                if (
                    ...
                    or (
                        model_type_list
                        and any(item in model_type_list for item in json.loads(model.model_type))
                    )
                )
                else False
            )
        ...
        payload = {"data": AIModelDto(filtered_list, many=True).data}
    ...
    def get_all_inference_log_list(
        self,
        project_id=None,
        page=1,
        data_per_page=5,
        ...
        if project_id:
            ...
            log_list = (
                InferenceLog.objects.filter(project_id=project.id, is_disabled=False)
                .order_by("-created_on")
                .all()
            )
        else:
    ...
    def get_timing_from_uuid(self, uuid):
        ...
        payload = {"data": TimingDto(timing).data}
    ...
    def get_timing_from_frame_number(self, shot_uuid, frame_number):
        ...
        if shot:
            timing = Timing.objects.filter(
                aux_frame_index=frame_number, shot_id=shot.id, is_disabled=False
            ).first()
            if timing:
                payload = {"data": TimingDto(timing).data}
    ...
    def get_primary_variant_location(self, uuid):
        ...
        payload = {"data": timing.primary_variant_location}
    ...
    def get_next_timing(self, uuid):
        ...
        next_timing = (
            Timing.objects.filter(
                aux_frame_index=timing.aux_frame_index + 1, shot_id=timing.shot_id, is_disabled=False
            )
            .order_by("aux_frame_index")
            .first()
        )

        payload = {"data": TimingDto(next_timing).data if next_timing else None}
    ...
    def get_prev_timing(self, uuid):
        ...
        payload = {"data": TimingDto(prev_timing).data if prev_timing else None}
    ...
    def get_timing_list_from_project(self, project_uuid=None):
        if project_uuid:
            ...
            timing_list = (
                Timing.objects.filter(shot_id__in=[s.id for s in shot_list], is_disabled=False)
                .order_by("aux_frame_index")
                .all()
            )
        else:
        ...
        payload = {"data": TimingDto(timing_list, many=True).data}
    ...
    def get_timing_list_from_shot(self, shot_uuid):
        ...
        timing_list = (
            Timing.objects.filter(shot_id=shot.id, is_disabled=False).order_by("aux_frame_index").all()
        )
    ...
    def create_timing(self, **kwargs):
        ...
        if "aux_frame_index" not in attributes.data or attributes.data["aux_frame_index"] == None:
            attributes._data["aux_frame_index"] = Timing.objects.filter(
                shot_id=attributes.data["shot_id"], is_disabled=False
            ).count()

        if "model_id" in attributes.data:
            if attributes.data["model_id"] != None:
                model: AIModel = AIModel.objects.filter(
        ...
        if "source_image_id" in attributes.data:
            if attributes.data["source_image_id"] != None:
                source_image: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=attributes.data["source_image_id"], is_disabled=False
                ).first()
                if not source_image:
        ...
        if "mask_id" in attributes.data:
            if attributes.data["mask_id"] != None:
                mask: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=attributes.data["mask_id"], is_disabled=False
                ).first()
                if not mask:
        ...
        if "canny_image_id" in attributes.data:
            if attributes.data["canny_image_id"] != None:
                canny_image: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=attributes.data["canny_image_id"], is_disabled=False
                ).first()
                if not canny_image:
        ...
        if "primary_image_id" in attributes.data:
            if attributes.data["primary_image_id"] != None:
                primay_image: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=attributes.data["primary_image_id"], is_disabled=False
                ).first()
                if not primay_image:
    ...
    def bulk_create_timing(self, timing_data_list):
        ...
        file_list = InternalFileObject.objects.filter(
            uuid__in=primary_image_id_list + source_image_id_list + mask_id_list + canny_image_id_list,
            is_disabled=False,
        ).all()
        ...
        payload = {"data": [TimingDto(timing).data for timing in res_timing_list]}
    ...
    def remove_existing_timing(self, project_uuid):
        ...
        if project:
            shot_list = Shot.objects.filter(project_id=project.id, is_disabled=False).all()
            Timing.objects.filter(shot_id__in=[s.id for s in shot_list], is_disabled=False).update(
                is_disabled=True
            )
    ...
    def add_interpolated_clip(self, uuid, **kwargs):
        ...
        if "interpolated_clip_id" in kwargs and kwargs["interpolated_clip_id"] != None:
            interpolated_clip: InternalFileObject = InternalFileObject.objects.filter(
                uuid=kwargs["interpolated_clip_id"], is_disabled=False
            ).first()
    ...
    def update_specific_timing(self, uuid, **kwargs):
        ...
        if "primary_image_id" in kwargs:
            if kwargs["primary_image_id"] != None:
                primary_image: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=kwargs["primary_image_id"], is_disabled=False
                ).first()
        ...
        if "source_image_id" in kwargs:
            if kwargs["source_image_id"] != None:
                source_image: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=kwargs["source_image_id"], is_disabled=False
                ).first()
                if not source_image:
                    return InternalResponse({}, "invalid source image uuid", False)
        ...
        if "mask_id" in kwargs:
            if kwargs["mask_id"] != None:
                mask: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=kwargs["mask_id"], is_disabled=False
                ).first()
                if not mask:
                    return InternalResponse({}, "invalid mask uuid", False)
        ...
        if "canny_image_id" in kwargs:
            if kwargs["canny_image_id"] != None:
                canny_image: InternalFileObject = InternalFileObject.objects.filter(
                    uuid=kwargs["canny_image_id"], is_disabled=False
                ).first()
                if not canny_image:
                    return InternalResponse({}, "invalid canny image uuid", False)
    ...
    def get_app_setting_from_uuid(self, uuid=None):
        ...
        payload = {"data": AppSettingDto(app_setting).data}
    ...
    def get_all_app_setting_list(self):
        ...
        payload = {"data": AppSettingDto(app_setting_list, many=True).data}
    ...
    def create_project_setting(self, **kwargs):
        ...
        if "default_model_id" in attributes.data and attributes.data["default_model_id"]:
            model = AIModel.objects.filter(
                uuid=attributes.data["default_model_id"], is_disabled=False
            ).first()
            if not model:
                return InternalResponse({}, "invalid model", False)
        ...
        if "audio_id" in attributes.data and attributes.data["audio_id"]:
            audio = InternalFileObject.objects.filter(
                uuid=attributes.data["audio_id"], is_disabled=False
            ).first()
            if not audio:
    ...
    def create_backup(self, project_uuid, backup_name):
        ...
        timing_list = (
            Timing.objects.filter(project_id=project.id, is_disabled=False).order_by("aux_frame_index").all()
        )
        ...
        for timing in final_list:
            ...
            timing["source_image_uuid"] = (
                str(id_file_dict[timing["source_image_id"]].uuid) if timing["source_image_id"] else None
            )
            ...
            timing["canny_image_uuid"] = (
                str(id_file_dict[timing["canny_image_id"]].uuid) if timing["canny_image_id"] else None
            )
            del timing["canny_image_id"]

            timing["primary_image_uuid"] = (
                str(id_file_dict[timing["primary_image_id"]].uuid) if timing["primary_image_id"] else None
            )
        ...
        payload = {"data": BackupDto(backup).data}
    ...
    def get_backup_list(self, project_uuid):
        ...
        payload = {"data": BackupListDto(backup_list, many=True).data}
    ...
    def get_shot_from_number(self, project_uuid, shot_number=0):
        ...
        shot: Shot = Shot.objects.filter(
            project_id=project.id, shot_idx=shot_number, is_disabled=False
        ).first()
        if not shot:
        ...
        payload = {"data": ShotDto(shot, context=context).data}
    ...
    def get_shot_list(self, project_uuid):
        ...
        shot_list: List[Shot] = (
            Shot.objects.filter(project_id=project.id, is_disabled=False).order_by("shot_idx").all()
        )
        timing_list = Timing.objects.filter(is_disabled=False).all()
        context = {"timing_list": timing_list}

        payload = {"data": ShotDto(shot_list, context=context, many=True).data}
    ...
    def duplicate_shot(self, shot_uuid):
        ...
        payload = {"data": ShotDto(new_shot, context=context).data}
